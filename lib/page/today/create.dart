...
class CreateTodayPage extends StatefulWidget {
  ...
  @override
  _CreateTodayPageState createState() => _CreateTodayPageState();
}

class _CreateTodayPageState extends State<CreateTodayPage> {
  ...
  Widget build(BuildContext context) {
    return ScaffoldUtils.defaultPage(
        RoutesConfig.createToday,
        Form(
            key: formKey,
            child: ScaffoldUtils.maxCenterBox(
                Column(
                  children: [
                    FontUtils.bodyTitle('あなたの趣味は'),
                    Container(
                      color: CupertinoTheme.of(_context).barBackgroundColor,
                      child: CupertinoFormRow(
                          child: TextFormField(
                        controller: favoriteController,
                        autofocus: true,
                        validator: Validator.settingTitle,
                        decoration: const InputDecoration(
                            fillColor: Colors.transparent,
                            filled: true,
                            hintText: '単語を入力　例)野球、キャンプ、料理、映画'),
                      )),
                    ),
                    SpaceUtils.smallY,
                    ...
                    SpaceUtils.normalY,
                    ButtonUtils.roundGradientButton(Icons.auto_awesome,
                        () async {
                      if (!CommonConfig.mine!.paid &&
                          CommonConfig.mine!.storyList.length >= 5) {
                        CommonConfig.noPaidSnackbar2('５記事以上の新規作成はできません');
                        return;
                      }
                      if (formKey.currentState!.validate()) {
                        String title = '今日の' + favoriteController.text;
                        StoryDBFirestore storyDBFirestore =
                        ...
                        await storyDBFirestore.createStory();
                        if (storyDBFirestore.uid == null) {
                          return;
                        }
                        CommonConfig.mine!
                            .addUserStoryList(storyDBFirestore.uid!);
                        Get.offNamed(RoutesConfig.storyToday.url, arguments: {
                          'docId': storyDBFirestore.uid,
                          'title': title
                        });
                      }
                    }, title: 'ページ作成', iconSize: 25, width: 150)
                  ],
                ),
                maxWidth: CommonConfig.toggleFormMaxWidth)),
        automaticallyImplyLeading: true);
  }
}

class _CreateTodayPageStatetest extends State<CreateTodayPage> {
  bool status1 = false;
  bool status2 = true;
  bool status3 = false;
  bool status4 = false;
